# Solving Algebraic Equations 解代数方程 {#algebra-solver.chapter}
...
Stan 提供了一种内置机制来指定代数方程组系统。这些方程组系统可以使用牛顿方法，如Kinsol包[@Hindmarsh:2005]中实现的那样，或鲍威尔混合方法[@Powell:1970]求解。Stan 用于代数方程求解器的函数署名在参考手册的代数求解器部分有完整的描述。
...
## Example: system of nonlinear algebraic equations例子：非线性代数方程系统
...
## Coding an algebraic system编码一个代数系统
...
代数方程组直接在Stan中编码为一个具有严格指定署名的函数。例如，上面给出的非线性系统可以使用下面的函数在Stan中进行编码（有关编码用户自定义函数的更多信息，请参阅用户自定义函数部分）。
...
## Calling the algebraic solver调用代数求解器
...
### Length of the algebraic function and of the vector of unknowns代数函数和未知向量的长度 {-}
...
## Control parameters for the algebraic solver代数求解器的控制参数 {#algebra-control.section}
...
最大步数可用于停止一个失控模拟。当发生了坏的跳跃，尤其是在预热阶段，MCMC可能会出现这种情况。如果达到限制，当前的提案将被拒绝。用户将看到一条警告消息，指出已超过最大步数。
