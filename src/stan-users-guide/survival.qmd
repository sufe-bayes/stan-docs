...
有了 Stan，我们只需将指数密度换成 Weibull 密度，再加上适当的参数，模型就基本不变了。回顾指数模型的参数和模型块。
...
比例危险模型不是完全生成的。由于基线危害函数 $h_0(t)$ 没有建模，因此无法生成故障时间；如果知道基线危害，就可以生成故障时间。比例危害模型可以生成以若干删减项目为条件的故障排序。
...
由于基线危险函数 $h_0(t)$ 未知，因此危险函数的比例规范不足以生成随机变量。另一方面，比例规范足以生成考虑生存时间顺序的部分似然。
