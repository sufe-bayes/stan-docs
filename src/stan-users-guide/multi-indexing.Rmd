...
# 多索引和范围索引 {#multi-indexing.chapter}
...
Stan允许在单个位置使用整数索引数组或范围边界来提供容器（即数组、向量和矩阵）的多个索引。在许多情况下，有一些函数可以提供类似的行为。
...
Stan可以只提供下限或上限。写成 `c[3：]` 就是 `c[3：size(c)]` 的简写形式。写成 `c[:5]` 就是 `c[1:5]` 的简写形式。
...
### 索引函数
Stan提供了`head`和`tail`函数，用于提取向量、行向量和一维数组的前缀或后缀。在每种情况下，返回类型与参数类型相同。例如，
...
将设置 b 等于 { a[5], a[6], a[7] }，因此它从a的第5个元素开始，包括了总共3个元素。
...
将左右两边的赋值表达式引用相同的数据结构会导致别名问题。在Stan中，赋值语句右边的表达式会被当作是一个全新的副本进行计算，因此在对同一数据结构进行赋值操作时，可能会产生预期之外的结果。例如，在上述例子中，由于右边的表达式被计算成一个全新的副本，所以最终`a`的值为$(5,5,6)$而不是$(5,5,5)$。
...
这个最后的例子阐明了一个重要的点：如果有一个下限索引，比如 `2:3`，其下限为 2，则在索引中加上下限减一，如上面的 `1 + j` 表达式所示。
...
此处将设置 `b` 等于从索引 [5, 9] 开始并延伸 3 行 2 列的矩阵子矩阵。因此 `block(a, 5, 9, 3, 2)` 相当于 `a[5:7, 9:10]`。
