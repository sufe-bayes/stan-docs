...
using the [`integrate_1d` function](https://mc-stan.org/docs/functions-reference/functions-1d-integrator.html).
...
如果两个极限都是 `负无穷()` 或 `正无穷( )`，则积分及其梯度都设为零。
...
积分采用 Boost 库（Agrawal 等人，2017 年）中的迭代一维求积方法。如果积分的第 n 次估计值用 $I_n$ 表示，积分的第 n 次估计值的范数用 $|I|_n$ 表示，则迭代在下列条件下结束
...
### 避免定积分中积分极限附近的精度损失{#integral-precision}
