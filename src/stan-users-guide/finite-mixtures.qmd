...
这个定义假设每个观测值 $y_n$ 都有可能来自混合模型中的任何一个组成部分。密度函数为
...
在许多混合模型中，模型内部的混合成分是可以互换的，这意味着我们无法明确区分它们。如果混合成分的参数具有相同（可交换）的先验分布，并且混合比率具有均匀先验分布，那么混合成分的参数在似然函数中也是可交换的。
...
我们通过对参数进行排序来解决这个基本问题。这种方法在某些情况下可以帮助我们识别出具体的混合成分，无论是在模型建立之前还是在模型拟合完成后（例如男性与女性，或民主党与共和党）。
...
联合分布 $p(y, \mu, \sigma, \lambda)$ 在参数 $(\mu_1, \sigma_1)$ 和 $(\mu_2, \sigma_2)$ 中是可交换的，其中 $\lambda$ 转换为 $1 - \lambda$。^[施加一个约束，如 $\theta < 0.5$，将解决对称性问题，但从根本上改变了模型及其后验推断。]
...
在混合成分不可识别的情况下，由于标签会在不同的 MCMC 链或不同的优化运行中切换或排列，因此诊断采样或优化算法的收敛性可能会很困难。幸运的是，不涉及特定成分标签的后验推断在标签切换下是不变的，可以直接使用。本小节考虑了一对示例。
...
给定完整的参数向量 $\theta$，对一个新观测 $\tilde{y}$ 的预测似然函数将是
