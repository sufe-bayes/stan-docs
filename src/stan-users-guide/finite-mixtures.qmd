...
有限混合模型中,我们假设最终的结果是从几个不同的分布中抽取出来的,而从哪个具体的分布中抽取,是由一个分类的混合分布模型来决定的。混合模型通常具有多峰密度，其峰值接近混合成分的峰值。混合模型可以通过多种方式进行参数化，以下各节所述。混合模型可以用于直接建模具有多峰分布的数据，也可以用作其他参数的先验。
...
在[聚类模型](#clustering.chapter)讨论过的聚类模型是一类特殊的混合模型，这类模型在工程和机器学习领域的聚类研究中被广泛应用。本章讨论的正态混合模型以多元形式重新出现，作为 $K$ 均值算法的统计基础；通常应用于聚类问题的潜在狄利克雷分布模型，可以视为混合成分版本的多项混合模型。
...
一种参数化混合模型的方法是，使用潜在分类变量来指定哪个混合成分决定结果。例如，考虑 $K$ 个正态分布，它们的位置参数是 $\mu_k \in \mathbb{R}$ ，尺度参数是 $\sigma_k \in (0,\infty)$。现在将它们按比例 $\lambda$ 混合，其中 $\lambda_k \geq 0$ and
...
这个模型不是 Stan 直接支持的，因为它涉及离散参数 $z_n$，但是 Stan 可以通过对 $z$ 求和，来采样 $\mu$ 和 $\sigma$，就像下一节描述的那样。
...
为了在 Stan 中实现前面概述的正态混合模型，可以将模型中的离散参数求和消除。如果 $Y$ 是由 $K$ 个正态分布混合而成的，这些正态分布的位置参数是 $\mu_k$ ，尺度参数是 $\sigma_k$，并且混合比例 $\lambda$ 位于单位 $K$ -单纯形上，则
...
如果 $a$ 和 $b$ 是对数尺度上的概率，那么 $\exp(a) + \exp(b)$ 是它们在线性尺度上的和，外部对数将结果转换回对数尺度；总结来说，log_sum_exp 在对数尺度上进行线性加法。使用 Stan 内置的 `log_sum_exp` 函数的原因是，它可以通过下面的计算方式防止在指数运算中发生下溢和上溢，
...
尺度参数数组 `sigma` 中的值被限制为非负，并在模型中给出了一个弱信息先验分布，选择这样的先验是为了避免零值产生的退化情况。
...
目前在 Stan 中没有一种在观察级别上向量化混合模型的方法。本节旨在警告用户不要尝试进行简单的向量化，因为它会导致不同的模型。我们在观察级别上定义了适当的混合模型，如下所示，其中我们假设 `lambda`, `y[n]`, `mu[1], mu[2]`, and
...
前一节中的正态混合模型，其中 $\theta =
(\mu, \sigma, \lambda)$，表明似然函数在标签交换下返回相同的密度，因此预测推断是可靠的。在 Stan 中，可以通过计算 $p(\tilde{y} \mid y)$（在有效样本量方面更加高效）或 $\tilde{y}$ 的抽样来进行预测推断，后者更容易插入其他推断。这两种方法都可以直接编写在程序的 generated quantities 块中。这里是直接（非采样）方法的一个例子。
...
与其他事件概率一样，这可以在 generated quantities 块中通过对 $z_i$ 和 $z_j$ 进行抽样并使用让它们相等的示性函数，或者通过计算积分内的项作为 generated quantities 来计算。与预测似然一样，计算期望比采样更具统计效率。
...
考虑以下关于零膨胀 Poisson 分布的例子。有一个概率 $\theta$ 观察到一个零，并且有一个概率 $1 - \theta$ 观察到一个 $\textsf{Poisson}(\lambda)$ 分布的计数（现在 $\theta$ 被用于混合比例，因为 $\lambda$ 是 Poisson 均值参数的传统符号）。给定概 率$\theta$ 和强度 $\lambda$，$y_n$ 的分布可以写成
...
上面给出的计算零膨胀 Poisson 分布的代码会冗余地计算所有 Bernoulli 项，每次第一个条件体执行时也会计算 `poisson_lpmf(0 | lambda)`。冗余项的使用取决于已知数据读入时的 `y`。这允许使用 transformed data 块来计算一些更方便的项，以在每次迭代中表达对数密度。
...
所有零和非零结果的边界条件都得到了适当处理；在矢量化情况下，如果 `y_nonzero` 为空，则 `N_nonzero` 将为零，代码最后两行 target 增量项将添加零。
...
其中 $\textsf{PoissonCDF}$ 是 Poisson 分布的累积分布函数，$1 - \textsf{PoissonCDF}(0 \mid \lambda)$ 是截断 Poisson（在0处截断）的相对归一化项。
...
则 transformed data 块能用来储存充分统计量
...
第一条语句解释了 Bernoulli 对零和非零计数的贡献。第二行是来自非零计数的 Poisson 贡献，现在已经矢量化。最后，截断的归一化是单独的一行，因此对数 CCDF 在0的表达式不会重复。还要注意，常量 `Ngt0` 取了相反数；尽可能保留子表达式常量，因为这样梯度就不必传播，直到遇到非常量项为止。
