...
标准优化算法，包括期望最大化（EM），在应用统计论文中通常用于描述最大似然估计算法。这些推导提供了编码 Stan 模型所需的边际化方法。
...
要在 Stan 中编码这个模型，必须边际化离散参数 $s$，以得到定义概率函数 $log \ p(e,l,D_t)$ 的模型。
完整的联合概率可以分解为
...
尽管变点模型是直接编码的，但用于 `s` 和 `t` 的双重嵌套循环在 `T` 上是二次的。Luke Wiklendt 指出，可以通过使用类似于隐马尔可夫模型的前向-后向算法的动态编程来实现线性替代；他提交了以下代码的稍微变种，用于替换上述Stan程序中的转换参数块。
...
使用默认配置的 NUTS 算法，可以轻松地使用 MCMC 拟合该模型。该模型收敛速度快，每两次迭代产生大约一个有效样本。因为这是一个相对较小的模型（在时间上的内部双重循环大约需要20,000步），所以运行速度很快。
...
因为 `lp` 被声明为转换参数，每个迭代中每个变点的 `lp` 值都是可用的。如果对 `lp` 的值不感兴趣，可以将其编码为模型块中的局部变量，从而避免了每次迭代保存值的 I/O 开销。
...
为确保 MLE 是 Lincoln-Petersen 估计，对 $N$ 使用了不恰当的均匀先验分布；如果可能的话，应该根据对研究种群的了解，用更具信息性的先验进行替代。
...
模型中的一个棘手部分是对种群大小 $N$ 设置的下界 $C - R + M$。低于此下界的值是不可能的，因为否则无法从重新捕获的 $C$ 只动物中抽 取 $R$ 个样本。实现此下界是为了确保在变换后的（非约束）空间上的参数支持无界，使得采样和优化能够以非约束的方式进行。在 $C$ 的声明中的下界意味着一个变量变换 $f : (C-R+M,\infty) \rightarrow (-\infty,+\infty)$，其中 $f(N) = \log(N - (C - R + M))$；参考手册中包含了所有约束参数变换的详细信息。
...
为了简化对潜在离散参数 $z_{i,t}$ 的边际化处理，Stan 模型依赖于一个派生量 $\chi_t$，用于表示在时间 $t$ 仍然存活的个体再次被捕获的概率（如果个体已经死亡，则再次捕获概率为零）。该数量通过递归方式定义如下：
$$
\chi_t
=
\begin{cases}
...
参数 $\phi_2$  和$p_3$，即时间2的死亡概率和时间3的捕获概率，是不可辨识的，因为两者都可以用来解释在时间3没有捕获到的情况。它们的乘积 $\beta_3 = \phi_2 \, p_3$ 是可辨识的。Stan 模型将 `beta3` 定义为生成的数量。不可辨识的参数对于 Stan 抽样器的自适应性会出问题。虽然在这里对自适应性的问题较小，因为参数是有界的，因此具有适当的均匀先验分布，但更好的做法是制定一个可辨识的参数化模型。一种方法是为 $p$ 和 $\phi$ 参数制定一个分层模型。
...
本节介绍的是 Cormack-Jolly-Seber（CJS）模型的个体级版本，而不是前一小节中的集体级模型。它还扩展了模型，允许任意数量的时间段。数据包括捕获事件数量 $T$，个体数量 $I$，以及一个布尔标志 $y_{i,t}$，指示个体 $i$ 是否在时间 $t$ 被观察到。在 Stan 中，
...
个体级 CJS 模型包含几个函数定义。前两个函数用于在转换的数据块中计算动物被捕获的第一个和最后一个时间段。^[另一种方法是在外部计算并将其作为预处理数据输入到 Stan 模型中。另一种可选的编码方式是使用稀疏表示，仅记录捕获事件以及它们的时间和被捕获的个体的标识。]
...
给定预先计算的量，模型块直接编码了 CJS 模型的对数似然函数。所有参数均保留其默认的均匀先验分布，模型简单地编码了在给定参数 `p` 和 `phi` 以及以 `p` 和 `phi` 定义的变换参数 `chi` 的情况下，观测值 `q` 的对数概率。
