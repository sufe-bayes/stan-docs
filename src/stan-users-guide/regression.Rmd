...
Aaron Goodman 在 stan 论坛上还提供了另一种方法，即在转化后的数据块上进行 QR 分解，然后转换成一个求和为零的参数 `x`，
...
这样就生成了一个每一个分量的边缘分布为标准正态分布的向量 `beta`，且通过 QR 分解使得求和为零。
...
其中，`inv(K)` 是 `1.0 / K` 的简写形式。由于 `beta_raw` 是一个单纯形向量，其总和为1，通过逐元素减去 inv(K)，可以确保其总和为零。由于单纯形元素的取值范围是有界的，因此需要一个缩放因子来使  `beta` 具有 K 个自由度，以便能够取得总和为零的所有可能值。
...
通过这个参数化方式，对参数 `beta_raw` 设置狄利克雷先验分布（Dirichlet prior），比如均匀分布。对 `beta_scale` 设置另一个先验分布，一般用于实现"缩减"（shrinkage）效果
