...
该模型假设噪声项服从标准差为`sigma`的正态分布。该模型对于两个回归系数使用了非正常先验分布。

### Matrix notation and vectorization {- #vectorization.section}

### 矩阵符号和向量化 {- #vectorization.section}
...
这种工作的另一个原因是，Stan的算术运算符被重载为在矩阵上执行矩阵运算。在这种情况下，因为`x`的类型是`vector`，`beta`的类型是`real`，所以表达式`beta * x`的类型是`vector`。由于Stan支持向量化，可以直接使用矩阵符号编写具有多个自变量的回归模型。
...
在`sigma`的声明中，约束条件`lower=0`将其值限制在大于等于0的区间上。在模型块中没有给出先验的情况下，将默认使用非正常的先验分布。
尽管我们可以添加更具信息量的先验分布，但是只要最终能够导出正常的后验分布，使用非正常的先验分布也是可接受的。
...
## QR重参数化 {#QR-reparameterization.section}
...
函数qr_thin_Q和qr_thin_R实现了薄QR分解，薄QR分解比使用qr_Q和qr_R得到的完整QR分解更可取，因为后者更容易耗尽内存（请参阅Stan函数参考手册，了解有关qr_thin_Q和qr_thin_R函数的更多信息）。在实践中，最好将其写成 $x = Q^\ast R^\ast$，其中 $Q^\ast = Q * \sqrt{n - 1}$，$R^\ast = \frac{1}{\sqrt{n - 1}} R$。因此，我们可以等价地写成 $\eta = x\beta = QR\beta = Q^\ast R^\ast\beta$。如果我们令 $\theta = R^\ast\beta$，那么我们有 $\eta = Q^\ast\theta$ 和 $\beta = R^{\ast^{-1}}\theta$。在这种情况下，前面的Stan程序变为：
...
由于这个Stan程序生成的预测值 $y$ 与之前的Stan程序生成的预测值相等，并且二者给出的 $\alpha$、$\beta$ 和 $\sigma$ 的后验分布也相同，许多人想知道为什么具有这种QR重参数化的版本在实践中表现得更好，通常在绝对时间和有效样本量方面都更好。原因有三点：
...
## 系数和尺度的先验分布 {#regression-priors.section}
...
然而，如在[QR重参数化章节](#QR-reparameterization.section)所述，如果你对回归系数的*位置*没有信息先验，那么最好对模型进行重参数化，使得回归系数成为输出量。在这种情况下，通常对重参数化后的回归系数使用何种先验并不重要，几乎任何随着结果变化的弱信息先验都可以使用。
...
## 逻辑回归和概率回归 {#logistic-probit-regression.section}
...
为了方便，我们通常定义一个中心化的参数向量$\beta$，即满足总和为零的约束条件，
$$
\sum_{k=1}^K \beta_k = 0.
$$
...
对于参数向量，有多种方式可以让它满足总和为零的约束条件，其中最高效的方式是将第$K$个元素定义为前$K-1$个元素的和的相反数。
...
在Stan论坛上，Aaron Goodman提供了以下代码，用于生成在`beta`各分量上具有标准正态边缘分布的先验，
...
Aaron Goodman在stan论坛上还提供了另一种方法，即在转化后的数据块上进行QR分解，然后转换成一个求和为零的参数`x`，
...
这样就生成了一个每一个分量的边缘分布为标准正态分布的向量`beta`，且通过QR分解使得求和为零。
...
另一种效率较低，但适用于对称先验的替代方法是对一个单纯形进行平移和缩放。
...
其中，`inv(K)`是`1.0 / K`的简写形式。由于`beta_raw`是一个单纯形向量，其总和为1，通过逐元素减去inv(K)，可以确保其总和为零。由于单纯形元素的取值范围是有界的，因此需要一个缩放因子来使 `beta`具有K个自由度，以便能够取得总和为零的所有可能值。
...
相较于使$\sum_{k=1}^K \beta_k = 0$（在其他条件相等的情况下），添加例如$\beta \sim \textsf{normal}(0,\sigma)$的先验提供了一种对参数向量$\beta$进行软中心化的方式。只有当$\beta$和$\beta + c$（$c$是标量常数）产生相同的似然（可能通过将另一个向量$\alpha$转化为$\alpha - c$，如在IRT模型中一样），这种方法才能保证能够大致中心化。这是实现对称先验的另一种方式。

## Ordered logistic and probit regression {#ordered-logistic.section}

## 有序logistic回归和probit回归 {#ordered-logistic.section}
...
有序回归包含输出$y_n \in \{ 1, \dotsc, k \}$和预测变量$x_n \in \mathbb{R}^D$，它由单个系数向量$\beta \in \mathbb{R}^D$和按顺序排列的截断点序列$c \in \mathbb{R}^{K-1}$确定。如果线性预测器$x_n \beta$的值落在$c_{k-1}$和$c_k$之间（假设$c_0 = -\infty$和$c_K = \infty$），则离散输出为$k$。噪声项由回归形式确定，这包括有序Logistic模型和有序Probit模型的示例。
...
有序Logistic模型可以使用Stan编码，其中使用`ordered`数据类型表示截断点，并使用内置的`ordered_logistic`分布。
...
通过将累积逻辑函数 (`inv_logit`) 替换为累积正态函数 (`Phi`)，可以以完全相同的方式编码有序Probit模型。
...
尽管基于 softmax 变换的内置编码更高效且数值上更稳定，逻辑回归模型也可以通过将 `Phi` 替换为 `inv_logit` 这种方式进行编码。通过计算值 `Phi(eta - c[k])` 一次并将其存储以供重复使用，可以获得一定的效率提升。
...
最简单的多层次模型是一种分层模型，其中数据被分组为 $L$ 个不同的类别（或层级）。一种极端的方法是完全汇集所有数据并估计一个共同的回归系数向量 $\beta$。另一种极端的方法是不进行汇集，为每个层级 $l$ 分别进行估计并分配其系数向量 $\beta_l$，。分层模型是一种中间解决方案，其中汇集程度由数据和对汇集数量的先验确定。
...
在可能的情况下，向量化采样语句可以加快对数概率和导数的计算速度。速度提升的原因不是因为消除了循环，而是因为向量化允许在对数概率和梯度计算中共享子计算，并且减少了梯度计算所需的表达式树的大小。
...
通过将伯努利分布中的逆对数概率应用替换为对数参数化的伯努利分布，可以进一步加快模型速度并提高算法稳定性。^[伯努利对数几率分布内置了对数链接函数，采用 $$\texttt{bernoulli}\mathtt{\_}\texttt{logit}\left(y \mid \alpha\right) = \texttt{bernoulli}\left(y \mid \operatorname{logit}^{-1}(\alpha)\right).$$]
...
与R或BUGS不同，Stan中的循环、数组访问和赋值速度很快，因为它们直接转化为C++代码。在大多数情况下，分配和赋值容器的成本都可以通过向量化的对数概率和梯度计算的提高效率来弥补。因此，以下版本比作为采样语句的循环版本更快。
...
在某些情况下，例如上述情况，局部变量赋值会导致模型可读性较差。在这种情况下，建议的做法是首先开发和调试更简单、更透明的模型版本，只有在简单版本调试完成后才进行优化工作。

## Hierarchical priors {#hierarchical-priors.section}

## 分层先验 {#hierarchical-priors.section}
...
在分层模型中，极大似然估计的基本问题是随着分层方差的减小和值聚集在分层均值周围，整体密度将无限增大。以一个简单的分层线性回归模型（具有固定的先验均值）作为例子，假设有 $y_n \in \mathbb{R}$ 和 $x_n \in \mathbb{R}^K$，模型可以表述为：
...
在这种情况下，当 $\tau \rightarrow 0$ 且 $\beta_k \rightarrow 0$ 时，后验密度 $p(\beta,\tau,\sigma|y,x)$ 将无限增大。可以参考 [Neal's funnel 密度的图表](#funnel.figure)，它展示了类似的行为。

在这种情况下，$\beta,\tau,\sigma$ 没有极大似然估计，因此如果要使用后验模型进行推断，必须修改模型。根据 @ChungEtAl:2013 推荐的方法，可以使用伽马分布作为 $\sigma$ 的先验，例如：

$$
\sigma \sim \textsf{Gamma}(2, 1/A)
$$

这里，$A$ 可以选择一个相对较大的值，比如 $A = 10$。通过选择这样的先验，可以在合理的 $\sigma$ 值周围集中更多的概率质量，减少对极端值的敏感性，从而解决密度无限增大的问题。

## Item-response theory models {#item-response-models.section}

## 项目反应理论模型 {#item-response-models.section}
...
项目反应理论（Item-Response Theory，IRT）模型用于描述一组学生中的每个学生对一组测试题目中一个或更多问题的回答情况。该模型基于学生的能力、题目的难度以及在更详细的模型中，例如题目的区分度和猜测正确的概率等参数。有关分层IRT模型的教材介绍，可以参考 @GelmanHill:2007 [pps. 314--320]，而关于在BUGS中编码各种IRT模型的内容，可以参考 @Curtis:2010。
...
为了考虑到并非每个学生都需要回答每个问题的情况，可以将提供的IRT模型数据声明如下所示：
...
为了简单起见，剩余部分的先验超参数将被硬编码，但在Stan中，它们也可以作为数据进行编码，以增加灵活性。

### 1PL (Rasch) model {-}

### 一参数logistic（Rasch）模型
...
The 1PL item-response model（一参数logistic模型，也称为Rasch模型）是一种经典的项目反应理论模型，该模型假设每个问题只有一个参数，且使用了逻辑斯蒂链接函数（L）。
...
在这里使用的非标准参数化还包括截距项 `delta`，它表示平均学生对平均问题的响应。^[@GelmanHill:2007 treat the $\delta$ term equivalently as the location parameter in the distribution of student abilities.]
...
理解这个模型的关键在于 `bernoulli_logit` 分布中的表达式，根据这个表达式可以得到
$$
\textsf{Pr}[y_n = 1] = \operatorname{logit}^{-1}\left(\alpha_{jj[n]} - \beta_{kk[n]} + \delta\right).
$$
...
为了进行测试，Stan中分发的IRT 1PL模型使用了具有信息量的先验，这些先验与在R中用于模拟数据的实际数据生成过程相匹配（模拟代码与模型位于同一目录下）。这对于大多数实际应用来说是不现实的，但可以用来验证Stan的推断结果。通过使用更宽松的先验进行简单的敏感性分析，可以发现即使在具有400个学生和100个问题，并且只有25%的随机缺失情况下，后验分布对先验相当敏感。在实际应用中，应该将先验与其他参数一起进行分层拟合，如下一节所述。
...
在本节中，我们将对上一节中描述的简单1PL模型进行推广，通过添加一个区分度参数来模拟问题的噪声程度，并为问题的难度和区分度参数添加多层先验。模型的参数声明如下：
...
2PL模型与1PL模型类似，但额外引入了参数`gamma[k]`来衡量问题`k`的区分度。如果`gamma[k]`大于1，则答案更加模糊，随机猜对的几率较低。我们限制参数`gamma[k]`为正数，这禁止了存在对低能力学生来说更容易的问题；虽然这种问题并非罕见，但它们在大多数应用IRT模型的测试情境中会被剔除。

在此模型中，学生能力`alpha`的先验分布为标准正态分布。这样做是为了确定参数的尺度和位置，否则它们将无法确定；有关可识别性的进一步讨论，请参阅[有问题的后验分布章节](#problematic-posteriors.chapter)。难度参数`beta`和区分度参数`gamma`在此模型中具有层次化的不同尺度。它们也可以给出弱信息的非层次化先验，例如
...
非中心参数化在分层模型中往往更高效；有关非中心参数化的更多信息，请参阅[重参数化部分](#reparameterization.section)。
...
使用 @AlbertChib:1993 引入的技巧，可以在Stan中编码多元probit回归，其中基础连续值向量$y_n$被编码为截断参数。在Stan中编码该模型的关键是将潜变量向量$z$分为两部分，根据相应的$y$值是0还是1。除此之外，该模型与前一节中的看似无关回归模型相同。
