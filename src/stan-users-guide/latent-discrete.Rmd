...
# 潜在离散参数 {#latent-discrete.chapter}

Translator: Fang Zhiqing
译者：方志清
...
Stan不支持对离散参数进行采样。因此，无法直接将具有离散参数（即离散随机节点）的BUGS或JAGS模型转换为Stan。尽管如此，通过边际化离散参数，可以对涉及有界离散参数的许多模型进行编码。[这些计算与期望最大化（EM）算法[@dempster-et-al:1977]中涉及的计算类似]。
...
本章介绍了如何编码涉及潜在离散参数的几个广泛使用的模型。下一章，[聚类章节](#clustering.chapter)，将进一步讨论涉及潜在离散参数的模型。

## The benefits of marginalization {#rao-blackwell.section}
## 边际化的好处 {#rao-blackwell.section}
...
尽管需要对联合概率函数进行一些代数运算，但同样需要计算的一个量是被边际化的变量的后验期望，这通常是模型感兴趣的量。因为使用了所有可能值的期望，而不是通过对离散参数进行抽样来估计期望，这样可以更好地探索分布的尾部，并且在每次迭代的基础上进行更有效的抽样。
...
标准优化算法，包括期望最大化（EM），在应用统计论文中通常用于描述最大似然估计算法。这些推导提供了编码Stan模型所需的边际化方法。

## Change point models {#change-point.section}
## 变点模型 {#change-point.section}
...
第一个例子是对英国1851年至1962年间的煤矿灾害进行建模。[数据的来源是[@Jarret:1979]，它本身是对早期数据收集的修正。]
...
最后一行使用条件运算符（也称为三元运算符），它借鉴自C语言和相关语言。条件运算符具有与C++中的对应运算符相同的行为。[R语言中的对应运算符`ifelse`稍有不同，它通常在矢量化的情况下使用。Stan中的条件运算符目前尚未矢量化。]
...
要在Stan中编码这个模型，必须边际化离散参数$s$，以得到定义概率函数$log \ p(e,l,D_t)$的模型。
完整的联合概率可以分解为
\begin{align*}
...
model calculations.
对数指数和函数使得模型可以直接在Stan中使用内置函数log_sum_exp来编码，它能够提供混合模型计算的算术稳定性和效率。
...
尽管变点模型是直接编码的，但用于`s`和`t`的双重嵌套循环在`T`上是二次的。Luke Wiklendt指出，可以通过使用类似于隐马尔可夫模型的前向-后向算法的动态编程来实现线性替代；他提交了以下代码的稍微变种，用于替换上述Stan程序中的转换参数块。
...
理解Wiklendt的动态规划版本的关键是要看到`head(lp_e)`保存了前向值，而`lp_l[T + 1] - head(lp_l, T)`保存了后向值；巧妙地使用减法使得`lp_l`可以在前向方向自然地累积。
...
it is  fast.
使用默认配置的NUTS算法，可以轻松地使用MCMC拟合该模型。该模型收敛速度快，每两次迭代产生大约一个有效样本。因为这是一个相对较小的模型（在时间上的内部双重循环大约需要20,000步），所以运行速度很快。
...
因为`lp`被声明为转换参数，每个迭代中每个变点的`lp`值都是可用的。如果对`lp`的值不感兴趣，可以将其编码为模型块中的局部变量，从而避免了每次迭代保存值的I/O开销。
...
constant),
给定一次迭代中`s`的值由迭代中的早期和晚期速率$e$和$l$给出。在收敛后的每个迭代中，通过MCMC采样从后验分布$p(e,l \mid D)$中抽取早期和晚期灾害率$e$和$l$，并计算相关的`lp`。根据当前的$e$和$l$的值，可以对`lp`的值进行归一化，以计算每个迭代中的$p(s \mid e,l,D)$。通过对迭代进行平均，可以得到变点为$s$的非归一化概率估计（归一化常数见下文），
...
其中$\texttt{lp}[m,s]$表示后验抽取$m$中变点$s$的`lp`值。通过对抽取进行平均，$e$和$l$本身被边际化处理，结果不依赖于给定迭代中$e$和$l$的值。最后的归一化操作产生了所关注的量，即在给定数据$D$的条件下，变点为$s$的后验概率，
...
使用Stan 2.4默认的MCMC实现计算得到的$\log p(s \mid D)$值的图示显示在后验图中。
...
与基于期望值的方法相比，离散采样效率非常低，特别是对于分布的尾部。因此，只有在需要明确从分布中绘制样本时，才应使用此方法。否则，应根据由`softmax(lp)`给出的变点后验分布在生成的量块中计算期望值。
...
可以使用对$s$进行的离散采样来计算与其他参数的协方差。尽管采样方法直观简单，但使用`lp`计算这些协方差的期望值在统计意义上更高效（即在相同的准确度下所需的迭代次数更少）。
...
第一个小节描述了一个简单的标记-重捕模型，不涉及任何潜在的离散参数。接下来的小节描述了Cormack-Jolly-Seber模型，该模型涉及动物死亡的潜在离散参数。
...
Lincoln-Petersen方法[@Lincoln:1930;@Petersen:1896]用于估计种群大小的公式为
$$
\hat{N} = \frac{M C}{R}.
$$
...
<a name="id:Lincoln-Petersen模型图"></a>
...
Lincoln-Petersen估计器的概率形式是基于一步标记-捕获研究的数据对种群大小进行估计。$N$的下界是必要的，以有效地排除不可能的值。
...
for this model.
Lincoln-Petersen估计器的概率变体可以直接在Stan中编码，如Lincoln-Petersen模型图所示。Lincoln-Petersen估计是该模型的最大似然估计（MLE）。
...
为确保MLE是Lincoln-Petersen估计，对$N$使用了不恰当的均匀先验分布；如果可能的话，应该根据对研究种群的了解，用更具信息性的先验进行替代。
...
模型中的一个棘手部分是对种群大小$N$设置的下界$C - R + M$。低于此下界的值是不可能的，因为否则无法从重新捕获的$C$只动物中抽取$R$个样本。实现此下界是为了确保在变换后的（非约束）空间上的参数支持无界，使得采样和优化能够以非约束的方式进行。在$C$的声明中的下界意味着一个变量变换$f : (C-R+M,\infty) \rightarrow (-\infty,+\infty)$，其中$f(N) = \log(N - (C - R + M))$；参考手册中包含了所有约束参数变换的详细信息。

### Cormack-Jolly-Seber with discrete parameter {-}
### 具有离散参数的Cormack-Jolly-Seber模型
...
这些参数都将具有均匀先验分布，但在实践中应使用信息来加优化这些先验分布。
...
distributed as
CJS模型还使用了一个隐含的离散参数$z_{i,t}$，用于指示每个个体$i$在时间$t$是否存活，其分布为
$$
...
conditional on $z$ as
该条件杜绝了模型中存在僵尸动物的可能性；一旦动物死亡，它就会一直保持死亡状态。在给定$z$的条件下，数据分布可以简单地表示为
...
为了简化对潜在离散参数$z_{i,t}$的边际化处理，Stan模型依赖于一个派生量$\chi_t$，用于表示在时间$t$仍然存活的个体再次被捕获的概率（如果个体已经死亡，则再次捕获概率为零）。该数量通过递归方式定义如下：
$$
\chi_t
=
\begin{cases}
...
基本情况是由于如果动物在最后一个时间段被捕获，那么它再次被捕获的概率为1，因为没有更多的捕获时间段。根据$\chi_{t+1}$定义$\chi_{t}$的递归情况包括两种可能性：(1) 不在下一个时间段存活的概率为$(1-\phi_t)$；(2) 在下一个时间段存活的概率为$\phi_t$，在下一个时间段未被捕获的概率为$(1-p_{t+1})$，并且在$t+1$期后继续存活的概率为$\chi_{t+1}$。
...
final column.
对于从未被捕获的动物，其历史记录为0，因为只有被捕获的动物才能被观察到。对于仅在最后一轮被捕获的动物，它们对CJS模型没有提供任何信息，因为捕获/未捕获的状态只有在先前的捕获条件下才具有信息价值。对于其余情况，对似然函数的贡献在最后一列中给出。
...
通过直接基于$\chi$来定义这些概率，就不需要使用一个潜在的二进制参数来指示动物在时间$t$是否存活。$\chi$的定义通常用于定义CJS模型的似然函数（即边际化潜在离散参数）。
...
Stan模型将$\chi$定义为基于参数$\phi$和$p$的转换参数。在模型块中，根据每个历史的计数递增对数概率。这第二步类似于将伯努利观测值汇总为二项式或将分类观测值汇总为多项式，只是在Stan程序中直接使用’target+=‘进行编码，而不是作为内置概率函数的一部分。
...
periods
以下是考虑个体在三个捕获时间段内被观察或未被观察的历史计数的Cormack-Jolly-Seber标记-再捕获模型的Stan程序：
...
参数$\phi_2$和$p_3$，即时间2的死亡概率和时间3的捕获概率，是不可辨识的，因为两者都可以用来解释在时间3没有捕获到的情况。它们的乘积$\beta_3 = \phi_2 \, p_3$ 是可辨识的。Stan模型将`beta3`定义为生成的数量。不可辨识的参数对于Stan抽样器的自适应性会出问题。虽然在这里对自适应性的问题较小，因为参数是有界的，因此具有适当的均匀先验分布，但更好的做法是制定一个可辨识的参数化模型。一种方法是为$p$和$\phi$参数制定一个分层模型。


### Individual Cormack-Jolly-Seber model {-}
### 个体级Cormack-Jolly-Seber模型 {-}
...
Stan,
本节介绍的是Cormack-Jolly-Seber（CJS）模型的个体级版本，而不是前一小节中的集体级模型。它还扩展了模型，允许任意数量的时间段。数据包括捕获事件数量$T$，个体数量$I$，以及一个布尔标志$y_{i,t}$，指示个体$i$是否在时间$t$被观察到。在Stan中，
...
个体级模型的优势在于可以添加影响存活或捕获概率的个体“随机效应”，以及避免在展开$2^T$个观测历史（对应于$T$个捕获时间）时涉及的组合问题。
...
个体级CJS模型包含几个函数定义。前两个函数用于在转换的数据块中计算动物被捕获的第一个和最后一个时间段。[另一种方法是在外部计算并将其作为预处理数据输入到Stan模型中。另一种可选的编码方式是使用稀疏表示，仅记录捕获事件以及它们的时间和被捕获的个体的标识。]
...
这两个函数用于在转换的数据块中定义每个个体的第一个和最后一个捕获时间。[如果输入数组表示的个体从未被捕获，这两个函数都返回0。没有捕获记录的个体对于模型的估计是无关的，因为所有的概率陈述都是基于先前的捕获条件的。通常情况下，它们会被从数据中删除，但是程序允许它们被包含在内，即使它们对对数概率函数没有贡献。]
...
转换的数据块还定义了`n_captured[t]`，它表示时间`t`的总捕获数。变量`n_captured`被定义为一个向量而不是整数数组，以便在生成的数量块中可以将其用于元素级的向量操作，从而对每个时间点的种群估计进行建模。
...
参数和转换参数与之前相同，但现在还有一个函数定义用于计算整个向量`chi`，即如果个体在`t`时还活着，则它将永远不会再次被捕获的概率。
...
给定预先计算的量，模型块直接编码了CJS模型的对数似然函数。所有参数均保留其默认的均匀先验分布，模型简单地编码了在给定参数`p`和`phi`以及以`p`和`phi`定义的变换参数`chi`的情况下，观测值`q`的对数概率。
...
外部循环是对个体进行的，条件是跳过从未被捕获的个体`i`。判断个体是否从未被捕获依赖于第一次捕获和最后一次捕获函数对于未被捕获的个体返回0的约定。
...
个体`i`的内部循环首先根据个体的存活概率`phi[t - 1]`增加对数概率。结果为1是固定的，因为个体必须在第一次和最后一次捕获之间存活（即不存在僵尸）。循环从第一次捕获后开始，因为CJS模型中的所有信息都是以第一次捕获为条件的。
...
在内部循环中，个体`i`在时间点`t`的观测捕获状态`y[i, t]`具有基于时间点`t`的捕获概率`p[t]`的伯努利分布。
...
`i` was observed.
在内部循环之后，还包括在个体`i`在时间`last[i]`被观察后再也没有被看到的概率，因为`last[i]`被定义为个体`i`最后一次被观察到的时间段。
...
与前面小节描述的集体模型类似，该模型无法确定`phi[T - 1]`和`p[T]`，但可以确定它们的乘积`beta`。因此，`beta`被定义为生成的量，用于监测收敛性和报告。
...
在生成的量中，还计算了在每个时间点`t`的种群均值估计，方法与简单标记-重捕模型相同，即将时间`t`捕获的个体数除以时间`t`的捕获概率。这是通过向量的元素级除法操作（`./`）在生成的量块中完成的。
...
predictors.
所有个体被建模为具有相同的捕获概率，但是这个模型可以很容易地推广到在个体级输入的基础上使用逻辑回归作为预测变量。


## Data coding and diagnostic accuracy models {#data-coding-models.section}
## 数据编码和诊断准确性模型 {#data-coding-models.section}
...
parameters.
在本节中，只考虑分类评级，并且Stan建模的挑战是要边缘化离散参数。
...
Dawid和Skene（1979）引入了一种嘈杂测量模型用于编码，并将其应用于编码医生对患者病史所说内容的流行病学设置中；相同的模型也可以用于诊断程序。
...
该模型的数据包括$J$个评级者（诊断测试）、$I$个项目（患者）和$K$个要进行注释的类别（疾病状态），其中$y_{i,j} \in \{1, \dotsc, K\}$表示评级者$j$对项目$i$的评级。在特定疾病的诊断测试环境中，评级者通常是诊断程序，而$K=2$，表示疾病的存在与否。[诊断程序通常是有序的，比如肿瘤诊断中的癌症阶段或牙科诊断中龋齿的严重程度。可以直接使用Dawid和Skene的模型，或者通过使用潜在连续评级和切点来自然地推广到有序评级，就像有序Logistic回归一样。]
...
once.
将Dawid和Skene的模型扩展到不是每个评级者都对每个项目进行精确评级的情况相对简单。
...
@DawidSkene:1979 在他们的期望最大化（EM）算法的E步中，推导出了完全相同的方程（2.7）。在Stan中，需要对边际化的概率函数进行对数变换。
...
以下是Dawid和Skene模型的Stan程序实现[@DawidSkene:1979]。
...
@DawidSkene:1979.
使用NUTS从模糊的初始点开始，Stan模型收敛迅速且混合效果良好，而使用Gibbs采样的等价模型无法达到相同的效果。合理的弱信息先验为$\alpha_k = 3$和$\beta_{k,k} = 2.5 K$，以及如果$k \neq k'$，则$\beta_{k,k'} = 1$。将$\alpha$和$\beta_k$视为单位向量，并应用优化，将得到与@DawidSkene:1979的期望最大化(EM)算法相同的结果。
...
`log_q_z[i]`被定义为一个转换参数。它编码了基于$\theta$和$\pi$的$p(z_i \mid \theta, \pi)$的（未标准化的）对数。每次迭代都会提供一个值，该值基于该迭代对$\theta$和$\pi$的值进行条件化。对`log_q_z[i]`应用softmax函数会提供与后验中$z_i$的概率质量函数相对应的单纯形。可以对这些值进行迭代平均，以获得关于每个$z_i$的后验概率分布。

## The mathematics of recovering marginalized parameters {#marginalization-mathematics.section}
## 从边缘化模型中恢复边缘化参数的数学原理{#marginalization-mathematics.section}
...
本节详细介绍了使用边缘化Stan模型的输出进行统计推断的数学原理，例如前三节中介绍的模型。它提供了关于为什么以及如何对Stan的输出进行某些操作以产生后验分布的有效摘要的数学解释，当统计模型中的离散参数被边缘化时。然而，完全理解本节中的数学内容*不是*使用Stan拟合具有离散参数的模型所必需的。
...
在进行贝叶斯推断时，通常关注的是估计后验分布的某些（常数维度的）概括统计量。数学上，我们对估计$\mu$感兴趣，其中$\mu = \mathbb{E}[g(\Theta, Z)]$，$g(\cdot)$是任意函数。一个例子是$\mathbb{E}[\Theta]$，即连续参数的后验均值，其中我们可以取$g(\theta, z) = \theta$。为了估计$\mu$，最常见的方法是从感兴趣的参数的后验分布中采样一系列值（至少近似地）。然后可以使用这些抽样的数值来计算感兴趣的量。通常，这个计算过程很简单，但在处理边缘化后验时需要更加小心，正如我们在本节中所描述的。
...
如果$\Theta$和$Z$都是连续的，可以使用Stan从联合后验分布$p_{\Theta, Z}(\theta, z)$中采样$M$个值，然后使用以下估计量来估计$\mu$：
...
然而，由于$Z$是离散的，Stan无法用于从联合后验中进行采样（甚至进行优化）。相反，如前几节所述的特定模型，用户可以首先从联合后验中边缘化$Z$，得到边缘化后验$p_\Theta(\theta)$。然后，可以像通常一样在Stan中实现这个边缘化后验，并且Stan将给出从边缘化后验中采样的抽样$\{\theta^{(i)}\}_{i = 1}^M$。
...
即使在边缘化之后，计算关于连续参数的函数的期望值也可以像没有进行边缘化一样进行计算
...
除了使用贝叶斯定理，$\Pr[Z = k \mid \theta = \theta^{(i)}, Y]$还可以通过在Stan模型中显式地包含条件概率来提取（就像[Dawid-Skene模型](#data-coding-models.section)一样）。
...
有关Stan中边缘化数学的更详细介绍，还涵盖了Rao-Blackwellization与边缘化之间的联系，请参阅@PullinEtAl:2021。
