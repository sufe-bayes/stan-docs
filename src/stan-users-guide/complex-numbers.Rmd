...
Stan在支持实数的标量、矩阵和向量的同时也支持复数的标量、矩阵和向量。


## Working with complex numbers使用复数
...
本节介绍复数标量的类型，包括如何构建复数、分配复数以及在数组和函数中使用复数。

### Constructing and accessing complex numbers构造和访问复数
...
生成复数 $-1.1 + 2.3i$。 这仅在实部和虚部是字面量时才有效。 要从任意实变量中构造一个复数，可以使用`to_complex()`函数。 例如，如果`x`和`y`是函数或模型块中的参数、转换数据或局部变量，则以下代码将起作用。
...
### Complex assignment and promotion复数的分配和晋升
...
复数数组允许对其元素进行赋值，整数或实数的赋值可以被提升为复数。


### Complex functions复数函数
...
提升也适用于复值函数的参数，这些参数可以接受整数或实数值，且将在计算函数之前被提升为复数。例如，以下用户定义的复函数将接受整数、实数或复数参数。
...
## Complex matrices and vectors复数矩阵和向量
...
## Complex linear regression复线性回归
...
问题仍然是如何给出误差分布。我们有几种选择方案，这里只考虑两种简单的方案，不考虑去惩罚误差的绝对值。



### Independent real and imaginary error独立的实部误差和虚部误差
...
### Dependent complex error相依的复数误差
...
在这里，误差的实部和虚部是具有相关和独立尺度的联合分布。该误差服从均值为零的多元正态分布和一个Cholesky因子协方差表示，其由尺度向量`sigma`和Cholesky因子或相关矩阵`L_Omega`构成。相关性的先验松散地分布在对角协方差附近，尺度上的先验未被指定。为了对`multi_normal_cholesky`的调用进行矢量化操作，需要将复数向量转换为大小为 2 的向量数组。
