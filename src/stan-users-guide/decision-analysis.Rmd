...
统计决策分析是关于在不确定性下做出决策。为了做出决策，结果必须与
某种“效用”概念相关联。所谓的“贝叶斯最优”决策是最大化期望效用
（或等价地，最小化期望损失）的决策。本章展示了如何使用Stan同时估计
基于决策的结果的分布并计算所需的期望效用。

## Outline of decision analysis 决策分析概要
...
decisions.
将所有可能的结果定义为集合$X$，将所有可能的决策定义为集合$D$。
...
结果应尽可能多地体现与效用相关的信息。在贝叶斯决策分析中，
结果的分布通常是一个给定观察数据的后验预测分布。在心理学和经济学中，
有大量有关定义效用函数的文献。例如，金钱的效用通常被认为是严格凹的，
而不是线性的
（即每多得到一单位的金钱的边际效用会随着金钱的增加而减少）。

## Example decision analysis 决策分析示例
...
本节介绍了一个非常简单的决策分析，用于帮助一位通勤者在步行、
共享单车、公共交通或出租车等不同交通方式中做出选择。
假设这位通勤者在过去一年中已经尝试了各种交通方式，
并且在这段时间内交通条件和费用都没有变化。在这一年内，
这个通勤者可能已经累积两百条关于不同通勤方式所需到达
工作地点的时间的观察数据。

### Step 1. Define decisions and outcomes 定义决策和结果 {-}
...
### Step 2. Define density of outcome conditioned on decision 定义以决策为条件的结果密度 {-}
...
\int
  ...
  \, \textrm{d}\theta.
$$ 
一年的通勤观测数据包括通勤方式的选择$d^{\textrm{obs}}_n$
和观测到的费用和时间
...
为简单起见，行程$n$的通勤时间t_n$将被建模为对数正态，
对于给定的交通方式选择$d_n \in 1:4，$ 
$$
t_n \sim \textrm{lognormal}(\mu_{d[n]}, \sigma_{d[n]}).
$$
为了理解符号$d_n$，也可写成$d[n]$，指行程$n$使用的交通工具。
例如，如果行程$n$是骑自行车的，那么
$t_n \sim \textrm{lognormal}(\mu_2, \sigma_2)，$
...
每一种交通工具$k \in 1:4，$都使用了简单、固定的先验， \begin{eqnarray*}
\mu_k & \sim & \textrm{normal}(0, 5)
\\[2pt]
\sigma_k & \sim & \textrm{lognormal}(0, 1).
\end{eqnarray*} 这些先验和广泛的通勤时间一致；在一个更现实的模型中，
基于对城市的了解和一天中的时间作为协变量，每种通勤方式都有自己的先验；
而此处通勤被认为是可交换的。
...
对于公共交通、步行和骑自行车来说，费用通常是一个常函数。然而，
为了简单起见，所有的成本将被建模为服从对数正态分布，
$$
c_n \sim \textrm{lognormal}(\nu_{d[n]}, \tau_{d[n]}).
$$ 
同样，交通工具的先验也都是固定的，
...
\\[2pt]
\tau_k & \sim & \textrm{lognormal}(0, 1).
\end{eqnarray*} 一个更现实的方法是以时间为条件来对费用建模，
因为出租车的费用取决于所选择的路线和所花费的时间。
...
### Step 3. Define the utility function 定义效用函数 {-}
...
为了使问题更具体，我们假设效用函数是成本和时间的简单函数。
进一步假设通勤者将通勤时间价值设定为每小时25美元，
效用函数关于通勤成本和时间是线性的。那么这个效用函数可以定义为 
$$
U(c, t) = -(c + 25 \cdot t).
$$ 
这个符号是负的，因为高成本是不可取的。一个更好的效用函数可能会
有一个阶梯函数或增加上班迟到的成本，不同的交通方式因其舒适度和
环境影响而有不同的成本，以及效用关于成本是非线性的。


### Step 4. Maximize expected utility 最大化期望效用 {-}
...
如果决策由少量的的离散选择组成，那么可以很容易地在Stan中编码期望效用。
效用函数被编码到functions，观测到的数据被编码到data，
模型参数被编码到parameters，
model模块本身被编码为服从每个参数的抽样分布。
```stan
functions {
...
generated quantities模块定义了一个数组变量`util`，其中`util[k]`保存了
...
为了简单起见，在最初的表述中，全部的四个通勤方案都有其成本估计，
尽管其中三个方案的成本是固定的。为了处理某些成本是固定的这一事实，
成本必须被硬编码或作为数据读入，`nu`和`tau`将被声明为单变量，
而成本的RNG只在`k == 4`时被采用。
...
为成对的向量定义效用函数将使generated quantities模块中的
随机数生成被向量化。
...
就Stan的执行来说，随机生成$x^{(m)}$是在从模型后验分布中抽取
$\theta^{(m)}$后，用`lognormal_rng`操作进行的。
在运行和组合多个链之后，计算平均值。
...
现在只需要做出具有最高期望效用的决策`k`，对应`util[k]`
的最高后验均值的选择。这可以从Stan的汇总统计中的`mean`栏中读出，
也可以通过Stan的接口以编程方式访问。
...
许多选择，例如为退休投资多少钱或在健身房花多长时间，都不是离散的，
而是连续的。在这些情况下，连续选择可以被编码为Stan程序中的数据。
然后可以计算期望效用。换句话说，Stan可以被用作从选择到期望效用的函数。
然后，外部优化器可以调用该函数。如果没有梯度信息，
这种优化可能会变得困难。梯度可以通过自动微分来提供，
但是Stan目前还不能计算这些导数。
