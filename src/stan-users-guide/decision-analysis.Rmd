...
统计决策分析是关于在不确定性下做出决策。为了做出决策，结果必须与某种“效用”概念相关联。所谓的“贝叶斯最优”决策是最大化期望效用（或等价地，最小化期望损失）的决策。本章展示了如何使用Stan同时估计基于决策的结果的分布并计算所需的期望效用。
...
用条件密度函数$p(x \mid d)$，$x \in X$且$d \in D.$，定义以决策为条件的结果的概率分布。
...
t_n \sim \textrm{lognormal}(\mu_{d[n]}, \sigma_{d[n]}).
$$
为了理解符号$d_n$，也可写成$d[n]$，指行程$n$使用的交通工具。例如，如果行程$n$是骑自行车的，那么
...
\\[2pt]
\sigma_k & \sim & \textrm{lognormal}(0, 1).
\end{eqnarray*} 这些先验和广泛的通勤时间一致；在一个更现实的模型中，基于对城市的了解和一天中的时间作为协变量，每种通勤方式都有自己的先验；而此处通勤被认为是可交换的。
...
\\[2pt]
\tau_k & \sim & \textrm{lognormal}(0, 1).
\end{eqnarray*} 一个更现实的方法是以时间为条件来对费用建模，因为出租车的费用取决于所选择的路线和所花费的时间。
...
为了简单起见，在最初的表述中，全部的四个通勤方案都有其成本估计，尽管其中三个方案的成本是固定的。为了处理某些成本是固定的这一事实，成本必须被硬编码或作为数据读入，`nu`和`tau`将被声明为单变量，而成本的RNG只在`k == 4`时被采用。
...
为成对的向量定义效用函数将使generated quantities模块中的随机数生成被向量化。
...
剩下的就是运行Stan。`util[k]`的后验均值是期望效用，用完整的条件写出来，就是
\begin{eqnarray*}
\mathbb{E}\!\left[U(x) \mid d = k, d^{\textrm{obs}}, x^{\textrm{obs}}\right]
& = &
\int
...
许多选择，例如为退休投资多少钱或在健身房花多长时间，都不是离散的，而是连续的。在这些情况下，连续选择可以被编码为Stan程序中的数据。然后可以计算期望效用。换句话说，Stan可以被用作从选择到期望效用的函数。然后，外部优化器可以调用该函数。如果没有梯度信息，这种优化可能会变得困难。梯度可以通过自动微分来提供，但是Stan目前还不能计算这些导数。
