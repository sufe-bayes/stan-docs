...
[高斯过程章节](#gaussian-processes.chapter)介绍了高斯过程，高斯过程也可以用于时间序列（和空间）数据分析。
...
带有正态噪声的一阶自回归模型（AR(1)）将序列 $y$ 中的每一个点 $y_n$ 按照以下规则生成：
$$
...
对于回归系数 $\alpha$ 和 $\beta$ 以及噪声尺度 $\sigma$（$\sigma$>0），给予不当（improper）扁平先验，AR(1) 模型的 Stan 程序如下。^[这个模型中的截距是 $\alpha / (1 - \beta)$。Mark Scheuerell 在 GitHub 上建议用截距 $\gamma$ 表示的另一种参数化方式是 $y_n \sim \textsf{normal}\left(\gamma + \beta \cdot (y_{n-1} - \gamma), \sigma\right)$。]
...
为了保证估计得到一个平稳的 AR(1) 过程，斜率系数 `beta` 可能会受到以下约束。
...
在实践中，这样的约束不建议使用。当不能通过一个平稳的 AR 模型来很好地拟合数据的时候，我们最好能知道这一点。不过在实践中可以通过设置 `beta` 先验质量集中在零的分布来鼓励稳定参数的估计。
...
这样的随机波动性模型的后验通常具有很大的后验方差。例如，用 $\mu = -1.02$、$\phi = 0.95$  和$\sigma = 0.25$ 模拟500个数据点，得到 $\mu$ 的95%后验区间为 $(-1.23, -0.54)$，$\phi$ 的为 $(0.82, 0.98)$，$\sigma$ 的 为$(0.16, 0.38)$。
...
使用 NUTS 的样本显示样本之间存在高度的自相关，这对于这个模型和在[@Hoffman-Gelman:2014]中评估的随机波动性模型都是如此。
