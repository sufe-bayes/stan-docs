...
distribution whose parameters are determined by the cluster 
assignment 
$z_n$,
...
The sample implementation in this section assumes a fixed 
unit 
...
$$
so that the log multivariate normal can be implemented 
directly up to a proportion
by
...
在Stan中，可以通过将有标签和无标签数据的模型结合起来，实现朴素贝叶斯模型的全贝叶斯后验推断。推断的目标包括模型参数和无标签数据的类别的后验分布。该模型实际上是一个缺失数据模型，假设未知的类别标签完全随机缺失。有关缺失数据处理的更多信息，@GelmanEtAl:2013和@GelmanHill:2007。该模型也是半监督学习的一个实例，因为未标记的数据有助于参数估计。
