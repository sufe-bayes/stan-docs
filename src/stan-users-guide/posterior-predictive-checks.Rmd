...
在一个后验预测检查中，超参数$\mu$和$\sigma$都来自后验分布，
$$
...
与先验预测检查一样，复制的值$\alpha$是从超参数中提取的，
$$
...
因此$y^{\textrm{rep}(m)}$可以看作是来自新群体观测的一种后验预测复制，而不是在原来的$K$组。

In Stan, mixed predictive replications $y^{\textrm{rep}(m)}$ can be
programmed directly.

在Stan中，混合预测复制量$y^{\textrm{rep}(m)}$可以被直接编程。
...
@GelmanEtAl:1996
，
先验重复、后验重复和混合重复都可以定义为联合模型在参数、观察和复制数据上的后验。
...
过载的抽样符号意味着$y$和$y^{\textrm{rep}}$来自同样的分布，或者更正式地使用大写字母来区分随机变量，条件分布$p_{Y^{\textrm{rep}} \mid
...
其中$y$是唯一可以被观测的变量，$\alpha$包含低水平参数，$\phi$是超参数。注意$\phi$没有被复制，而是出现在$\alpha$和$\alpha^{\textrm{rep}}$的分布中。
