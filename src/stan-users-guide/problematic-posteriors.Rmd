...
# 有问题的后验分布 {#problematic-posteriors.chapter}
...
从数学角度来说，有了一个适当的后验分布，就可以进行贝叶斯推断，就这样。甚至不需要要求有有限的方差或有限的均值---只需要有一个有限的积分。然而，建模是一项棘手的任务，即使是经验丰富的模型构建者有时也会编写导致不恰当的先验分布的模型。此外，有些后验分布在数学上是合理的，但在实践中表现不佳。本章讨论了在贝叶斯推断中或在Stan中实践中创建有问题的后验推断的模型中存在的问题。
...
## 回归中预测变量的共线性 {#collinearity.section}
...
本节讨论与经典的可辨识性概念相关的问题，这会导致后验密度中的岭，并对采样和推断造成严重影响。
...
第一个共线性的示例是一个涉及冗余截距参数的人为示例。^[这个例子是Richard McElreath在Stan用户组中提出的，他在关于BUGS和JAGS中使用的Gibbs采样与Stan中使用的Hamiltonian Monte Carlo（HMC）和no-U-turn samplers（NUTS）之间行为差异的查询中提出的。]

第一个共线性的例子是一个涉及冗余截距参数的人造例子。^[这个例子是Richard McElreath在Stan用户组中提出的，他询问了BUGS和JAGS中使用的Gibbs采样与Stan使用的Hamiltonian Monte Carlo (HMC)和no-U-turn采样器 (NUTS)之间的行为差异。]
...
假设有$n \in \{1,\dotsc,N\}$的观测值$y_n$，两个截距参数$\lambda_1$和$\lambda_2$，一个尺度参数$\sigma > 0$，以及采样分布
...
对于任意常数$q$，如果我们将$q$加到$\lambda_1$并从$\lambda_2$中减去它，即
...
这导致一个不正确的均匀先验 $p(\mu,\sigma) \propto 1$ 导致了一个不正确的后验分布。这种不正确性是因为无论 $q$ 是什么，$\lambda_1 + q, \lambda_2 - q$ 周围的邻域具有相同的质量。因此，采样器需要在 $\lambda_1=1\,000\,000\,000$ 和 $\lambda_2=-1\,000\,000\,000$ 的邻域中花费与在 $\lambda_1=0$ 和 $\lambda_2=0$ 的邻域中一样多的时间，以此类推，对于更远距离的值也是如此。
...
对于该模型，边缘后验分布 $p(\lambda_1,\lambda_2 \mid y)$ 是不正确的。^[在至少有两个不同数据点的情况下，$\sigma$ 的边缘后验分布 $p(\sigma \mid y)$ 是正确的。]

因此，这个模型的边际后验分布$p(\lambda_1,\lambda_2 \mid y)$是不适当的。^[只要至少存在两个不同的数据点，这里的边缘后验分布 $p(\sigma \mid y)$ 对于 $\sigma$ 是合适的。]
...
这种不正确性在后验密度中以岭的形式可视化，如左图所示。该模型的岭沿着 $\lambda_2 = \lambda_1 + c$ 的直线, 对于某个$c$。
...
的简单回归模型相比，即使具有不正确的先验，后验仍然是正确的，只要至少有两个具有不同值的数据点 $y_n$。
...
考虑一个针对学生$j \in 1{:}J$的项目反应理论（IRT）模型，其中包括能力$\alpha_j$和测试项目$i \in 1{:}I$的难度$\beta_i$。观测数据是一个$I \times J$的数组，其中的元素$y_{i, j} \in \{ 0, 1 \}$表示学生$j$正确回答问题$i$。数据的抽样分布为
...
对于任意常数$c$，在所有能力上加$c$并从所有难度中减去它不会改变$y$的概率，即
...
这导致了前面讨论的具有两个截距的回归模型所展示的岭的多变量版本。
...
当回归模型的预测变量具有共线性时，就会出现共线性问题的一般形式。例如，考虑一个线性回归的抽样分布
...
其中$y$是$N$维观测向量，$x$是$N \times K$的预测变量矩阵，$\beta$是$K$维系数向量。
...
现在假设预测变量矩阵的第$k$列是第$k'$列的倍数，即存在某个常数$c$使得对于所有$n$都有$x_{n,k} = c \, x_{n,k'}$。在这种情况下，系数$\beta_k$和$\beta_{k'}$可以共变而不改变预测结果，即对于任意$d \neq 0$，
...
即使预测变量矩阵的列不完全共线，如果它们几乎共线，也会导致类似的推断问题。
...
考虑在IRT模型中为每个问题添加一个歧视性参数$\delta_i$，其数据抽样模型为
...
对于任意常数$c \neq 0$，将$\delta$乘以$c$并将$\alpha$和$\beta$除以$c$会产生相同的似然函数，
...
如果$c < 0$，则会改变$\alpha$、$\beta$和$\delta$的所有分量的符号，但不会改变密度。
...
为了参数化一个$K$-simplex（即，一个非负值和为一的$K$维向量），只需要$K - 1$个参数，因为第$K$个参数可以通过将前$K - 1$个参数的和减一得到，因此如果$\theta$是一个$K$-simplex，则有
...
`softmax`函数将线性预测器的$K$维向量$\alpha$映射到一个$K$-simplex$\theta = \texttt{softmax}(\alpha)$，其定义为
...
`softmax`函数是多对一的，这导致了无约束参数$\alpha$的不可辨识性。特别地，对每个$\alpha_k$加减一个常数会产生相同的simplex $\theta$。
...
前面讨论的所有例子都允许参数的平移或缩放，同时保持数据概率密度不变。可以通过多种方式来缓解这些问题。
...
对于具有多个截距$\lambda_1$和$\lambda_2$的情况，最简单的解决方案是去除冗余的截距，得到一个只有一个截距参数$\mu$和抽样分布$y_n \sim \textsf{normal}(\mu, \sigma)$的模型。对于解决共线性问题，同样的解决方案也适用——只需去除预测变量矩阵$x$的其中一列即可。
...
没有区分参数的IRT模型可以通过将其中一个参数固定到一个固定值（通常为0）来修复。例如，可以将第一个学生的能力参数$\alpha_1$固定为0。现在，所有其他学生的能力参数可以解释为相对于学生1的能力。类似地，难度参数相对于学生1能够回答的难度进行解释。
...
这种解决方案无法解决由问题区分参数$\delta_i$引入的乘法不变性问题。为了解决这个问题，必须对其中一个难度参数进行约束，比如将$\delta_1$约束为一个非零值，1是一个方便的选择。现在，所有的区分参数可以相对于第一个项目的区分进行解释。
...
$\texttt{softmax}(\alpha)$的多对一特性通常通过固定$\alpha$的一个分量来缓解，例如将$\alpha_K$固定为0。得到的映射是从$K-1$个无约束参数到$K$-simplex的一对一映射。这大致是Stan中简单约束参数的定义方式；详细定义请参阅约束参数转换的参考手册章节。从$K-1$维向量创建一个simplex的Stan代码可以编写为：
...
迄今为止，我们讨论的模型假设参数的先验概率是不正确的均匀先验概率。
...
其中$\tau$是一个常数标度，确保后验模式位于$\lambda_1 = \lambda_2$的点上，因为这最小化了$\log \textsf{normal}(\lambda_1 \mid 0,\tau) + \log
\textsf{normal}(\lambda_2 \mid 0,\tau)$。^[Laplace先验（或用于惩罚最大似然估计的L1正则项）不足以消除这种加法不变性。它提供了收缩，但并不能自动识别参数，因为在$\lambda_1$上加上一个常数并从$\lambda_2$上减去该常数会得到相同的先验密度值。]

通过在多个截距参数上使用具有常数尺度$\tau$的正态先验分布，可以确保后验模式位于使得$\lambda_1 = \lambda_2$的位置，因为这最小化了$\log \textsf{normal}(\lambda_1 \mid 0,\tau) + \log \textsf{normal}(\lambda_2 \mid 0,\tau)$。^[一个拉普拉斯先验（或者L1正则项用于惩罚最大似然估计）并不足以消除这种加法不变性。它可以提供收缩效果，但本身并不能识别参数，因为将一个常数加到 $\lambda_1$ 上并从 $\lambda_2$ 中减去相同的常数会导致先验密度得到相同的值。]
...
下面的图示展示了没有先验概率的两个截距参数化、具有标准正态先验概率的两个截距参数化以及没有先验概率的一个截距重新参数化的后验概率。对于这三种情况，后验概率是基于从标准正态分布中抽取的100个数据点绘制的。
...
两个截距参数化导致了一个不正确的先验概率，形成一个无限延伸到西北和东南的岭。
...
对于两个截距模型添加先验的结果显示在第二张图中；最后一张图显示了重新参数化为单一截距的结果。
...
另一种识别$K$-simplex参数化$\theta = \texttt{softmax}(\alpha)$的策略是在$\alpha$的各个分量上放置一个具有固定位置的先验概率（特别是避免具有可变位置的分层先验概率）。与将$\alpha_K = 0$固定不同，基于先验的方法对$K$个结果进行对称建模，而不是相对于第$K$个结果对$K-1$个结果进行建模。另一方面，固定参数化通常在统计上更有效，因为它没有额外的（由先验约束的）灵活性。
...
在解决不变性问题时，添加先验概率需要谨慎。如果先验概率选择得过于宽泛（即模糊），则解决方案仅在理论上成立，抽样器仍然会面临困难。
...
理想情况下，应根据对所建模问题的实质性了解来制定现实的先验概率。可以根据先前知识选择适当的强度来选择这样的先验概率。如果存在强有力的先验信息，那么选择具有强信息量的先验概率是有意义的。
...
当没有强先验信息时，弱信息量的先验概率能够在控制计算推断的同时不主导后验数据，找到合适的平衡点。在大多数问题中，建模者至少对估计值的预期范围有一定的概念，并且能够选择一个先验概率，用于识别目的，不会主导数据，但能对后验数据提供足够的计算控制。
...
先验概率也可以以同样的方式用于控制IRT模型的加法不变性。一种典型的方法是在学生能力参数 $\alpha$ 上放置强先验概率，以控制尺度，从而控制基本IRT模型的加法不变性和扩展了项目辨别参数的乘法不变性；这样的先验概率不会为问题添加任何先前知识。然后，可以根据问题的先前知识选择对项目难度的先验概率，这可以是具有信息量或弱信息量的先验。
...
## 混合模型中的标签交换问题 {#label-switching-problematic.section}
...
考虑一个带有两个位置参数 $\mu_1$ 和 $\mu_2$、共享尺度 $\sigma > 0$、混合比例 $\theta \in [0,1]$ 和似然函数的正态混合模型：
...
随着混合成分数量 $K$ 的增加，问题会加剧，例如在聚类模型中，导致 $K!$ 个相同的后验最大值。
...
对于混合模型，后验收敛性和有效样本量的分析也很困难。例如，Stan 报告的 $\hat{R}$ 收敛统计量和有效样本量的计算都会受到标签交换的影响。问题在于后验均值，这些计算中的关键要素受到了标签交换的影响，导致 $\mu_1$ 的后验均值等于 $\mu_2$ 的后验均值，而 $\theta$ 的后验均值始终为 1/2，无论数据是什么。
...
在某种意义上，混合成分的指标（或标签）是无关紧要的。即使不确定混合成分的身份，后验预测推断仍然可以进行。例如，新观测的对数概率不依赖于混合成分的身份。在这种情况下，唯一可靠的贝叶斯推断是对标签交换具有不变性的推断。参数的后验均值是没有意义的，因为它们对标签交换不具有不变性；例如，在两个成分的混合模型中，$\theta$ 的后验均值总是 1/2。
...
从理论上讲，这对于推断来说不应该是一个问题，因为后验预测推断中涉及的所有积分都会表现良好。实际上的问题在于计算。
...
在实践中能够进行这种不变推断是完全不同的问题。即使找到一个后验模式也几乎是难以解决的，更不用说根据概率质量平衡多个局部最大值的邻域的探索了。在 Gibbs 采样中，当在当前的 $\mu_2$ 和 $\theta$ 的值给定的条件下对 $\mu_1$ 进行采样时，$\mu_1$ 移动到新模式的概率是很低的。对于 HMC 和 NUTS，问题在于采样器陷入了围绕模式的两个"碗"之一，并且无法从随机动量分配中获得足够的能量来从一个模式转移到另一个模式。
...
即使对于适当的后验分布，已知的所有采样和推断技术在模式的数量以超指数方式增长时（随着混合模型的成分数量增加），都因为效果不佳而闻名。
...
为了解决实际中标签交换问题所带来的困难，提出并采用了几种"技巧"。
...
一种常见的策略是对参数进行约束，以确定各个成分。例如，在上面讨论的两个成分的正态混合模型中，我们可以考虑对参数 $\mu_1$ 和 $\mu_2$ 施加约束 $\mu_1 < \mu_2$。这种方法可能会导致一个问题，即当相反排序 $\mu_1 > \mu_2$ 的概率质量较大时。在这些情况下，后验受到约束的影响，对 $\mu_1$ 和 $\mu_2$ 的真实后验不确定性无法被带有约束的模型捕捉到。此外，用于事件概率的后验推断的标准方法也会受到影响。例如，试图使用 $M$ 个后验样本估计 $\textsf{Pr}[\mu_1 > \mu_2]$ 将会失败，因为估计量
...
另一种常见方法是运行单个链或将参数初始化在合理的值附近。^[退火方法可以视为自动执行此类模式搜索的方法，尽管大多数 MCMC 退火方法仍在不断进行模式搜索；详见 [@SwendsenWang:1986; @Neal:1996b]。]

另一种常见的方法是运行单个链或将参数初始化为接近真实值的值。^[温度调节方法可以被视为自动化的方式来进行这种模式搜索，尽管大多数MCMC温度调节方法会持续进行模式搜索；参考[@SwendsenWang:1986; @Neal:1996b]。]
...
如果能够找到合理的初始值并且标签在马尔可夫链中不会交换，这种方法比硬约束方法更有效。结果是，所有链都与后验的某个特定模式的附近粘合在一起。
...
在混合模型中，两个混合成分在采样或优化过程中可能会合并为相同的值。例如，$K$ 个正态分布的混合模型可能会演变为 $\mu_i = \mu_j$ 和 $\sigma_i = \sigma_j$（其中 $i \neq j$）。
...
这通常会在采样的早期阶段发生，这是由于MCMC或优化的初始化，或者由于MCMC过程中的随机移动而产生的。一旦参数在给定的抽样 $(m)$ 中匹配，就很难逃离，因为当前参数值和没有合并成分的参数值之间可能存在低密度的凹槽。
...
在预热阶段使用较小的步长，对每个混合成分的成员责任使用更强的先验，可能有助于解决这个问题。一个更极端的措施是增加额外的混合成分，以处理其中一些成分可能合并的情况。
...
一般来说，当 $K$ 增加到超过1时（是的，即使是两个成分的混合模型也可能存在此问题），在混合模型中精确恢复正确的 $K$ 个混合成分是困难的。
...
在某些情况下，随着参数接近某些极点或边界，后验密度会无限增长。在这种情况下，不存在后验模式，并且当采样参数接近约束边界时，可能会出现数值稳定性问题。
...
另一个无界密度的例子是后验分布 $\textsf{beta}(\phi \mid 0.5, 0.5)$，当对没有数据的群体使用看似弱的贝塔先验时，可能会出现这种情况。当 $\phi \rightarrow 0$ 和 $\phi \rightarrow 1$ 时，该密度是无界的。同样，与"弱"贝塔先验结合的伯努利似然模型会导致后验分布为
...
如果 $N = 9$ 且每个 $y_n = 1$，后验分布将为 $\textsf{beta}(\phi \mid 9.5,0.5)$。当 $\phi \rightarrow 1$ 时，该后验分布是无界的。尽管没有后验模式，但后验均值是明确定义的，并且其值恰好为 0.95。
...
Stan在这个问题上不直接在受限制的$(0,1)$空间上进行采样，因此不直接处理非受限制的密度值。相反，概率值$\phi$经过逻辑(logit)变换转换为$(-\infty,\infty)$空间。0和1处的边界被推到$-\infty$和$\infty$。Stan自动应用的Jacobian调整确保了非受限制的密度是合适的。对于$(0,1)$的特定情况，调整为$\log \operatorname{logit}^{-1}(\phi) + \log \operatorname{logit}(1 - \phi)$。

在这个问题中，Stan方法并不直接在受限制的$(0,1)$空间上采样，因此它不直接处理无限制的密度值。相反，概率值$\phi$经过logit变换后落在$(-\infty,\infty)$区间。边界0和1被推到$-\infty$和$\infty$。Stan自动应用的Jacobian调整确保了无限制的密度是合适的。对于特定情况$(0,1)$，调整为$\log \operatorname{logit}^{-1}(\phi) + \log \operatorname{logit}(1 - \phi)$。
...
然而，仍然存在两个问题。首先，如果$\phi$的后验质量接近其中一个边界，逻辑变换后的参数将需要扫过很长的路径，从而可能主导无转弯条件（no-U-turn sampler, NUTS）所施加的条件。第二个问题是，从非受限制空间到受限制空间的逆变换可能会下溢到0或上溢到1，即使非受限制的参数并非无穷大。类似的问题也会在逻辑回归的期望项中出现，这就是为什么伯努利分布和二项分布的logit-scale参数化更稳定的原因。
...
在某些情况下，后验密度不会无限增长，但是参数会随着密度值的逐渐增加而无限增长。与前一节讨论的具有无界密度增长的模型类似，这样的模型也没有后验模式。
...
现在假设预测变量矩阵的第$k$列满足当且仅当$y_n = 1$时$x_{n, k} > 0$的条件，这个条件被称为“可分离性”。在这种情况下，对于$y_n = 1$的情况，$x_n \beta \rightarrow \infty$，因此$\operatorname{logit}^{-1}(x_n \beta) \rightarrow 1$，导致在观测数据上的预测准确性会随着$\beta_k \rightarrow \infty$而继续提高。
...
在可分离性情况下，似然函数没有最大值，因此没有最大似然估计。从贝叶斯的角度来看，后验分布是不合适的，因此$\beta_k$的边缘后验均值也没有定义。贝叶斯模型中通常解决这个问题的方法是为$\beta$引入一个合适的先验分布，以确保合适的后验分布。
...
假设你的模型包含一个参数$\psi$，它在$[0, 1]$上定义，并且被赋予一个均匀先验$\textsf{uniform}(\psi \mid 0, 1)$。现在，如果数据对$\psi$没有提供任何信息，后验分布也将是$\textsf{uniform}(\psi \mid 0, 1)$。
...
尽管$\psi$没有最大似然估计，但后验分布在闭区间上是均匀的，因此是合适的。在均匀后验分布$[0, 1]$上，$\psi$的后验均值被定义为$1/2$。尽管没有后验模式，但通过对$[0, 1]$中所有点的$\psi$的预测进行积分（平均），后验预测推断仍然可以得到正确的结果。
...
对于不合适的后验分布，理论上是不可能进行正确的后验探索的。然而，通过BUGS和JAGS执行的Gibbs抽样虽然无法正确地从这样的不合适的后验分布中抽样，但在实践中与Stan执行的Hamiltonian Monte Carlo抽样在面对像[多重共线性部分](#collinearity.section)中讨论的两个截距模型并在非可识别密度图中展示的例子时表现出不同的行为。
...
通过BUGS和JAGS执行的Gibbs抽样在这个不可识别模型中可能表现出高效和良好的行为，但正如前面的小节中讨论的那样，它实际上无法正确地探索后验分布。
...
考虑初始值$\lambda_1^{(0)}, \lambda_2^{(0)}$。Gibbs抽样在迭代$m$中通过抽取进行
...
现在考虑对于$\lambda_1$的抽样（$\lambda_2$的抽样是对称的），在这个模型中，由于当前的$\lambda_2$和$\sigma$的值，下一个$\lambda_1$可以抽取的范围受到高度限制。Gibbs抽样将快速运行，并为$\lambda_1 + \lambda_2$提供看似合理的推论。但它不会探索后验的全部范围；它只会从初始值开始进行缓慢的随机漫步。这种随机漫步行为是Gibbs抽样在后验相关性较高的情况下的典型行为，也是选择Hamiltonian Monte Carlo而不是Gibbs抽样进行具有后验相关性的参数模型的主要原因。
...
Stan执行的Hamiltonian Monte Carlo (HMC)在探索参数后验分布中具有相关性的模型中更加高效。在这个特定的例子中，Hamiltonian动力学（即在由负对数后验定义的场中给定随机动量的虚拟粒子的运动）将沿着由势能定义的山谷上下运行（log后验的脊线对应势能的山谷）。在实践中，即使对于$\lambda_1$和$\lambda_2$有一个随机的动量，log后验的梯度将调整相关性，并且模拟将使$\lambda_1$和$\lambda_2$沿着对应于后验密度函数中的脊线相反的方向运行。

Hamiltonian Monte Carlo (HMC)，如Stan所执行的那样，在处理后验中参数相关的模型时具有更高的效率。在这个特定的例子中，Hamiltonian动力学（即，在由负对数后验定义的场中给予随机动量的虚拟粒子的运动）将沿着由势能定义的山谷上下运行（log后验中的岭对应势能中的山谷）。实际上，即使对于$\lambda_1$和$\lambda_2$随机给定动量，对数后验的梯度也会调整以适应它们之间的相关性，从而使得模拟中的$\lambda_1$和$\lambda_2$沿着对应于后验密度中岭的山谷相反的方向运行。
...
Stan的默认no-U-turn抽样器（NUTS）在探索后验分布方面更加高效[参见@Hoffman-Gelman:2014]。NUTS模拟了代表参数值的虚拟粒子的运动，直到它产生一个转向，但在大多数情况下，它将被击败，因为它只会无限地沿着势能山谷向下移动而不产生转向。在实践中，许多迭代中将达到模拟中的最大蛙跳步数，导致大量的对数概率和梯度评估（如果将最大树深度设置为10，则为1000次，与默认值相同）。因此，抽样将表现出较慢的特点。这表明后验是不合适的，而不是NUTS算法或其实现中的错误。从不合适的后验中抽样是不可能的！因此，HMC的行为以及NUTS的行为应该让人放心，因为在后验不合适的情况下，它们会明显失败，并在后验中扫除大路径以提供清晰的诊断信息。
...
以下是使用默认参数拟合生成的$N=100$个数据点$y_n \sim \textsf{normal}(0,1)$的Stan运行结果：
...
顶部是具有不合适的均匀先验和似然函数$y_n \sim \textsf{normal}(\lambda_1 + \lambda_2, \sigma)$的非可识别模型。
...
中间是具有与中间模型相同的似然函数以及先验分布$\lambda_k \sim \textsf{normal}(0,10)$。
...
底部是一个具有不合适的先验、似然函数$y_n \sim \textsf{normal}(\mu,\sigma)$的可识别模型。所有模型都在约0.16左右估计出$\mu$，蒙特卡洛标准误较低，但后验标准差较高，为0.1；真实值$\mu=0$在这三个模型的90%后验区间内。
...
为了说明从非可识别模型和仅具有弱标识的模型中采样的问题，我们拟合了三个模型，其参数的可识别程度逐渐增加。这些模型的后验分布在非可识别密度图中进行了说明。第一个模型是在[共线性部分](#collinearity.section)讨论的没有先验的两个位置参数的非可识别模型。
...
第二个模型在先前模型的模型块中为`lambda1`和`lambda2`添加了先验。
...
这三个示例模型均使用Stan 2.1.0的默认参数进行拟合（1000个预热迭代、1000个抽样迭代，NUTS采样器，最大树深度为10）。结果显示在非可识别拟合图中。这些输出的关键统计数据如下所示。
...
* 如`R_hat`列所示，在非可识别模型中，除了$\lambda_1$和$\lambda_2$之外，所有参数都已收敛。
...
* 在可识别模型中，平均蛙跳步数约为3，使用弱先验标识的模型约为150，在非可识别模型中约为1400。
...
* 每秒有效样本数对于$\mu$在可识别模型中约为31,000，在使用弱信息先验进行标识的模型中约为1,900，在非可识别模型中约为200；$\sigma$的结果类似。
...
* 在非可识别模型中，$\lambda_1$的95%区间为(-2300,6000)，而在使用弱信息先验进行标识的模型中仅为(-12,12)。
...
* 在这三个模型中，模拟得到的$\mu=0$和$\sigma=1$均在后验的90%区间内。
...
前两点，收敛不足和达到最大蛙跳步数（等价于最大树深度）都表明后验分布是不合适的。因此，与Gibbs采样器可能通过低质量采样来掩盖问题不同，Hamiltonian Monte Carlo试图探索后验分布，而其失败明确表明模型存在问题。
