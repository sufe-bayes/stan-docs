# Held-Out Evaluation and Cross-Validation  自动评估与交叉验证 
翻译者：沈梓梁
...
Held-out评估包括将数据集分成两部分，
其中一个训练数据集和另一个测试数据集.
训练数据集用于估计模型，而测试数据集用于评估.
...
交叉验证涉及通过以不同方式划分单个数据集来执行重复的Held-out评估.
...
Held -out评估和交叉验证可能涉及任何类型的预测统计量，
常见的选择是测试数据的预测对数密度、参数估计的均方误差或分类任务的准确性.

## Evaluating posterior predictive densities 评估后验预测密度 {#evaluating-posterior-predictive.section}
...
指数对数和计算完成的必须在Stan的外部使用 
$\log p(\tilde{y} \mid \tilde{x},\theta^{(m)})$后验绘图.
...
以下是在给定预测因子$\tilde{x}$和训练数据$(x, y)$
的简单线性回归中评估测试数据$\tilde{y}$的对数后验预测密度的代码
...
给定来自Stan的$M$后验绘图，序列`log_p[1:M]`将可用，
因此给定训练数据和预测器的测试数据的对数后验预测密度只是
`log_sum_exp(log_p) - log(M)`.
...
### Predictive estimates in Stan Stan中的预测估计
...
在Stan中，只有两个步骤中的第一步(重要的方差缩减步骤)可以在对象模型中编码.
线性预测器在生成数量块中定义.
```stan
data {
  int<lower=0> N_tilde;
...
Stan计算的`tilde_y`的后验均值是贝叶斯估计$\hat{\tilde{y}}.$
...
并且可以很容易地在Stan中实现.缺点是结果估计的方差比使用平衡块分区的方差要大.

### Stan implementation with random folds Stan用随机折叠的实现
...
这将限制其用于转换数据和生成数量块.
该代码遍历向量，将每个项与另一个随机选择的项交换，从而得到整数的统一排列
`1:N`.^[传统的方法是遍历一个向量，并用剩余元素中的随机元素替换每一项，
...
转换后的数据块使用置换RNG通过获取置换数据的前缀和后缀来生成训练数据和测试数据.
```stan
data {
  int<lower=0> N;
  vector[N] x;
...
因为测试集是常数，期望算子是线性的，
所以Stan程序中定义的`err`的后验均值将是后验均值估计的误差，
\begin{eqnarray*}
...
### Cross-validation with structured data 使用结构化数据进行交叉验证
...
### Cross-validation with spatio-temporal data 使用时空数据交叉验证
...
### Approximate cross-validation 近似交叉检验
...
以较低的成本逼近留一交叉验证的评估.
该方法在R包 loo 中有文档记录和实现[@GabryEtAl:2019].
