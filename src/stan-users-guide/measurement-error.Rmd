...
统计模型中使用的大多数的量都是由测量所得。而大多数的测量都有一定的误差。当相对于被测量的量，测量误差很小时，它对模型的影响通常很小。当测量误差相对于被测量的量来说很大时，或者当可以估计被测量量之间的精确关系时，引入一个显式的测量误差模型是有用的。一种测量误差就是舍入（rounding）。
...
从统计的角度来看元分析很像测量误差模型，其中从多个数据集中得出的推断被结合，来整体的对它们进行推断。对每个数据集的推断都被当作是提供了一种相对于真实参数值的测量误差。
...
测量误差的贝叶斯方法可以通过将被测量的真实量视为缺失数据来直接构建[@Clayton:1992; @RichardsonGilks:1993]。这需要一个关于如何从真实值中获取测量值的模型。
...
现在假设预测变量 $x_n$ 的真实值是未知的，但是对于每个 $n$，有一个 $x_n$ 的测量值 $x^{\textrm{meas}}_n$ 是可用的。如果测量的误差可以被建模，那么测量值 $x^{\textrm{meas}}_n$ 可以用真实值 $x_n$ 加上测量噪声（noise）来建模。真实值 $x_n$ 被视为缺失数据，与模型中的其他量一起被估计。一个简单的方法是假设测量误差是正态的，标准差 $\tau$ 已知。由此得到下面含常测量误差的回归模型。
...
一种常见的测量误差形式来自于对测量结果进行舍入。舍入可以用许多方式进行，例如将重量舍入到最接近的毫克或最接近的磅，甚至可以通过向下舍入到最接近的整数来进行。
...
舍入过程意味着 $z_n \in (y_n - 0.5, y_n + 0.5)$。离散观测值 $y$ 的概率质量函数可以通过边缘化消去未舍入的测量值得到，从而得到似然
...
Gelman 解决这个问题的方法是令无信息先验在对数尺度上关于方差 $\sigma^2$ 是均匀的，由于 Jacobian 调整，这会产生先验密度函数： 
...
或者，模型也可以使用潜参数（latent parameters）来定义未经舍入的测量值 $z_n$。在这种情况下，Stan 代码直接使用 $z_n$ 的似然函数来保证 $z_n \in (y_n - 0.5, y_n + 0.5)$ 这一约束条件。由于 Stan 不允许对向量（或数组）的元素设置不同的上下界约束条件，因此将参数设定为舍入误差 $y - z$，然后将 z 定义为转换后的参数。
...
元分析旨在汇总来自多个研究的数据，例如在多所学校中应用辅导计划或在多项临床试验中使用药物进行治疗。
...
贝叶斯框架特别适用于元分析，因为每个先前的研究都可以被看作是对某些感兴趣的潜在量的有误差的测量。然后，模型直接来源于两个组成部分，即对感兴趣的潜在量的先验和对每个被分析的研究的测量误差式模型。
...
临床数据由 $J$ 个试验组成，每个试验包含 $n^t$ 个治疗组病例，$n^c$ 个对照组病例，治疗组中有 $r^t$ 个成功结果，对照组中有 $r^c$ 个成功结果。可以在 Stan 中如下声明这些数据。^[Stan的整数约束不足以表达约束条件 $\texttt{r}\mathtt{\_}\texttt{t[j]} \leq \texttt{n}\mathtt{\_}\texttt{t[j]}$，但这个约束条件可以在 transformed data 模块中被写入。]
...
虽然临床试验数据在其原始格式下是二项的，但可以将其转换到无穷尺度上，通过考虑对数几率（log odds ratio）
\begin{align*}
...
对数几率和标准误可以在 transformed data 模块中定义，但需要注意不要使用整数除法 （integer division）。^[当对两个整数进行除法运算时，结果类型为整数，并且如果结果不精确，则会进行舍入。更多相关信息，请参阅参考手册中原始算术类型的讨论。]
...
如果可以使用特定于试验的预测变量，可以将其直接包含在用于每个试验治疗效果 $\theta_j$ 的回归模型中。
