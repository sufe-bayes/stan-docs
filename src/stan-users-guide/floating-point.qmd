...
在$\mathbb{R}$语言中，计算机会使用固定的位数来近似实数值。本章将解释这是如何实现的，以及为什么编写稳健的Stan（和其他数值）程序会非常重要。研究计算机上实数算术工作原理的计算机科学子领域被称为*数值分析*。
...
Stan的算术运算是使用双精度浮点数实现的。大多数现代计算机的算术规则^[在某些优化设置下，英特尔的优化编译器是一个显著的例外。]遵循*IEEE浮点数算术标准*（IEEE 754）。
...
IEEE 754标准中的双精度部分指定使用64位（8字节）的固定模式表示实数值。所有的值都是以基数为2的形式（即二进制）表示的。该表示形式由两个有正负之分的部分构成：
...
一个*正规*的浮点数值在尾数中不会使用零作为第一位；而*次正规*数第一位可能会使用零。并非所有的I/O系统都支持次正规数。
...
为了使合法的指数值范围在$-(2^{10}) + 2 = -1022$和$2^{10} - 1 = 1023$之间，会有一些预设的指数值。合法的尾数值介于$-2^{52}$和$2^{52} - 1$之间。浮点数允许表示非常大和非常小的值。一些极端值包括：
...
由于符号位的存在，有两种表示零的方式，通常被称为“正零”和“负零”。这种区别在Stan中是无关紧要的（就像在R中一样），因为这两个值是相等的(即`0 == -0`被认为是正确的)。
...
有一种特别的位模式被用于表示非数字值（在编程语言的输出中通常写作NaN，包括Stan）。
...
Stan提供了一个值函数`not_a_number()`，它返回这个特殊的非数字值。它用于表示错误条件而不是缺失值。通常，当非数字是一个函数的参数时，如果没有输出异常（在Stan中是拒绝输出），则结果将是非数字。

Stan also provides a test function `is_nan(x)` that returns 1 if `x`
is not-a-number and 0 otherwise.

Stan还提供了一个测试函数`is_nan(x)`，如果x是非数字，则返回1，否则返回0。
...
在几乎所有数学运算中，非数字值都会传播。例如，所有的内置二元算术运算（加法、减法、乘法、除法、不等于运算）如果它们的任何一个参数是非数字，则返回非数字。内置函数如`log`和`exp`也具有该传播非数字值的行为。

Most of Stan's built-in functions will throw exceptions (i.e., reject)
when any of their arguments is not-a-number.

Stan的大多数内置函数在它们的任何一个参数是非数字时将输出异常（即拒绝）。
...
与非数字的比较会始终返回false，包括与自身的比较。也就是说，`not_a_number() == not_a_number()`会返回false，尽管这会有些令人困惑。这就是为什么在Stan（和C++）中需要一个内置的`is_nan()`函数。唯一的例外是不等于运算，它仍然连贯。这意味着`not_a_number() != not_a_number()`返回true。
...
还有两个特殊值用于表示正无穷（$\infty$）和负无穷（$-\infty$）。它们不像非数字那样病态，但通常用于表示上溢和下溢等错误条件。例如，`log(0)`不会引发错误或返回非数字，而是求值为负无穷。对负无穷进行指数化将返回零，因此会有`0 == exp(log(0))`。然而，在Stan中不应该这样做，因为用于计算导数的链式法则将尝试非法操作并返回非数字。

There are value functions `positive_infinity()` and
`negative_infinity()` as well as a test function `is_inf()`.

Stan中有`positive_infinity()`和`negative_infinity()`两个值函数，以及一个测试函数`is_inf()`。
...
正无穷和负无穷具有符合预期的比较行为，因此对`negative_infinty() < 0`求值的结果为true（在Stan中用1表示）。此外，对正无穷取反会得到负无穷，反之亦然。
...
在像Stan这样的编程语言中，数字可以用标准的十进制（基数为10）表示法来表示。例如，`2.39`或`-1567846.276452`。请记住，编写超过16个有效数字的做法是没有意义的，因为它们无法被表示。可以使用*科学计数法*来编码Stan中的数字，它由带符号的基数的十进制表示和带符号的整数十进制指数组成。例如，`36.29e-3`表示的数字是$36.29 \times
...
对尾数的选择提供了$\log_{10} 2^{53} \approx 15.95$个十进制（基数为10）数字的算术精度。这只是浮点表示的精度。在多个操作链接在一起后，实际的算术精度通常要低得多。
...
这突显出了精度取决于尺度。尽管`1 + 1e-20 == 1`，但我们有`1e-20 + 1e-20 == 2e-20`，正如预期的那样。
...
舍入也表现为缺乏*传递性*。特别地，对于三个浮点数$a, b, c$，$(a + b) + c = a + (b + c)$通常是不成立的。
...
在统计应用中，当用户期望实数运算的常规规则能够成立是，问题会经常显现出来。假设我们有一个具有严格正对角线的下三角矩阵$L$，使得它是正定矩阵$L \, L^{\top}$的乔里斯基因子。在实际应用中，舍入和精度损失可能导致结果$L \, L^{\top}$既不对称也不正定。
...
在实践中，必须小心防止舍入误差。例如，可以通过将$L \, L^{\top}$与其转置相加并除以二，或者将下三角部分复制到上半部分来产生对称性。正定性则可以通过在对角线上加一个小量来保持。
...
基于这个原因，*机器精度*被认为大约是$10^{-15.95}$。在Stan中，该常量可以使用`machine_precision()`函数获得。
...
对于接近1时数字四舍五入的问题，可以通过一些特殊的操作来减轻。例如，考虑对正数`x`进行操作`log(1 + x)`。当`x`很小时（小于双精度浮点数的$10^{-16}$），参数中的和将会四舍五入为1，结果将会四舍五入为零。为了提供更多的精度，编程语言提供了一个库函数来直接实现$f(x) = \log (1 + x)$。在Stan中（与C++相同），这个操作被写作`log1p(x)`。由于`x`本身可能接近于零，函数`log1p(x)`可以取接近于1的值的对数，其结果接近于零。
...
浮点数表示法的另一个缺点是，当两个接近的数相减时会造成精度损失，这取决于它们的接近程度。这在实践中很容易看出。例如，考虑以下计算：
\begin{align*}
  1&.23456789012345 \\
- 1&.23456789012344 \\
= 0&.00000000000001
...
我们开始时具有15位小数的准确性，结果只剩下一位小数的准确性。
...
在统计计算中，若一个分布具有相对于其位置而言很小的标准差，当我们计算该分布的方差时，会出现抵消灾难。在计算统计量时，Stan使用Welford算法来计算方差。这避免了抵消灾难，并且可以在单次遍历中完成。
...
即使`1e200`可以表示为双精度浮点值，也没有足够大的有限值可以表示`1e200 * 1e200`。`1e200 * 1e200`的结果被称为*上溢*。IEEE 754标准要求结果为正无穷大。
...
在统计计算中，上溢很少是一个问题。如果是的话，可以在对数尺度上进行运算，就像下面描述的下溢一样。
...
当没有足够小的数字可以表示一个结果时，就称为*下溢*。例如，`1e-200`可以表示，但是`1e-200 * 1e-200`会下溢，结果为零。
...
下溢是似然计算中普遍存在的问题。例如，如果$p(y_n \mid \theta) < 0.1$，那么一旦$N > 350$或出现关于类似的数值
$$
p(y \mid \theta) = \prod_{n=1}^N p(y_n \mid \theta)
$$
就会发生下溢。
...
为了处理下溢，可以在对数尺度上进行运算。尽管无法表示$p(y \mid
\theta)$，但我们可以表示
$$
\begin{array}{rcl}
\log p(y \mid \theta)
...
尽管在引入指数算术时可能会出现上溢的情况，但真正评估时并不按照原始表达式逐项计算。相反，通过一些代数运算，将这些项重新排列成一种稳定的形式，
$$
\textrm{log-sum-exp}(u, v)
=
...
尽管内部项本身可以使用内置函数`log1p`进行求值，但是这只会带来有限的收益，因为当 
$u$远远大于$v$时， $\exp(v - u)$会接近零，这意味着最终的结果很可能会四舍五入为$u$。

To conclude, when evaluating $\log (a + b)$ given $\log a$ and $\log
b$, and assuming $\log a > \log b$, return

综上所述。当给定$\log a$和$\log b$来求解$\log (a + b)$时，假设$\log a > \log b$，会返回
$$
\log (a + b) =
...
Log-sum-exp函数能够以明显的方式推广到序列，即如果$v = v_1, \ldots, v_N$，那么
\begin{eqnarray*}
\textrm{log-sum-exp}(v)
...
一个很直接的应用是在对向量$u$的均值进行计算时完全使用对数尺度。也就是说，给定$\log u$并返回$\log \textrm{mean}(u)$。
\begin{eqnarray*}
...
由于浮点数表示是不精确的，因此测试精确不等式并不是一个好的提议。一般的建议是，在测试`x == y`时，可以使用一个给定的绝对或相对容差进行近似测试。

Given a positive *absolute tolerance* of `epsilon`, `x` can be compared
to `y` using the conditional

给定一个正的*绝对容差*`epsilon`，可以使用条件语句将`x`与`y`进行比较
...
当`x`和`y`的尺度以及相关的比较已知时，绝对容差可以起作用。

Given a positive *relative tolerance* of `epsilon`, a typical
comparison is

给定一个正的*相对容差*`epsilon`，一个典型的比较方法是
