# Proportionality Constants 比例常数  {#proportionality-constants.chapter}
翻译者：沈梓梁
...
在MCMC、变分推理或优化问题的对数密度计算中，需要评估似然或先验的一部分，
通常只需要计算到成常数比例的函数(或者类似地计算到相差常数的对数密度).
在MCMC中，这是因为被采样的分布不需要归一化(因此忽略的是归一化常数).
...
## Dropping Proportionality Constants 不考虑比例常数的情况
...
## Keeping Proportionality Constants 考虑比例常数的情况
...
## User-defined Distributions 使用者定义的分布
...
## Limitations on Using `_lupdf` and `_lupmf` Functions 使用`_lupdf`和`_lupmf`函数的限制
