# 上一位的同学把 Latent Discrete Parameters 都翻译完了，所以我把这一章都翻译完。
...
The first example of collinearity is an artificial example involving redundant intercept parameters.^[This example was raised by Richard McElreath on the Stan users group in a query about the difference in behavior between Gibbs sampling as used in BUGS and JAGS and the Hamiltonian Monte Carlo (HMC) and no-U-turn samplers (NUTS) used by Stan.]
...
Suppose there are observations $y_n$ for $n \in \{1,\dotsc,N\}$, two intercept parameters $\lambda_1$ and $\lambda_2$, a scale parameter $\sigma > 0$, and the sampling distribution
