...
因此，这个模型的边际后验分布$p(\lambda_1,\lambda_2 \mid y)$是不适当的。
...
通过在多个截距参数上使用具有常数尺度$\tau$的正态先验分布，可以确保后验模式位于使得$\lambda_1 = \lambda_2$的位置，因为这最小化了$\log \textsf{normal}(\lambda_1 \mid 0,\tau) + \log \textsf{normal}(\lambda_2 \mid 0,\tau)$。
